...
プラグインオブジェクトのリセット (再描画) のたびに GitHub の main ブランチの
最新コミットを確認し、インストール済みのコミットと異なれば VectorWorks 設定
フォルダ内の Python Externals フォルダへ更新インストールしてから、プラグイン
本体を実行する。
本体パッケージは純 Python のためアーカイブの直接展開でインストールし
(VectorWorks 同梱の Python では pip を実行できない場合があるため pip に
依存しない)、certifi 等の依存ライブラリだけを pip で揃える。
...
PACKAGE_NAME = "vectorworks-plugin-column-under-mark"
MODULE_NAME = "vectorworks_plugin_column_under_mark"
# pip でインストールする依存ライブラリ (pyproject.toml の dependencies と
# 同期させること)。本体は certifi 以外の外部依存を持たないが、certifi は
# main.py の自動更新が VectorWorks 同梱 Python (CA 証明書バンドル非同梱の
# 場合あり) から HTTPS アクセスするために使う
DEPENDENCIES = ("certifi",)
REPOSITORY = "min-nano/vectorworks-plugin-column-under-mark"
...
def _is_python_interpreter(candidate: str) -> bool:
    ...
    応答しないプロセスは kill して棄却する。依存ライブラリのバイナリ
    wheel が VectorWorks 内蔵 Python で使えるよう、バージョン
    (major.minor) の一致も要求する。
    """
...
def _purge_cached_modules(externals: str, *, purge_dependencies: bool = False) -> None:
    ...
    依存ライブラリ (certifi 等) も破棄する。依存を更新した場合
    (初回インストール・修復時) のみ True にすること。C 拡張は
